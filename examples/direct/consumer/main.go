...
func (h *handler) Process(body []byte, msgID string) error {
	log.Printf("direct consumer recv: msgID=%s body=%s", msgID, string(body))

	if string(body) == "fail" {
		return errors.New("mock direct consume error")
	}
...
func (h *handler) Failed(msg rabbit.FailedMsg) {
	log.Printf("direct consumer failed: msgID=%s body=%s", msg.MsgId, string(msg.Message))
}
...
func main() {
	...
	mq, err := rabbit.NewConsumeDirect(
		...
		rabbit.WithConnectionName("demo-direct-consumer"),
		rabbit.WithQueueName("demo.direct.queue"),
