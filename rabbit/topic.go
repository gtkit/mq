...
func (r *MqTopic) Consume(handler MsgHandler) error {
	...
	for {
		...
		if err != nil {
			if waitErr := r.waitRetry("consume", &retryAttempt, "topic consumer open channel failed: %v, reconnecting...", err); waitErr != nil {
				return waitErr
			}
			continue
		}
		...
		if err != nil {
			closeAMQPChannel(ch)
			if waitErr := r.waitRetry("consume", &retryAttempt, "topic consumer declare queue failed: %v, reconnecting...", err); waitErr != nil {
				return waitErr
			}
			continue
		}
		...
		if err != nil {
			closeAMQPChannel(ch)
			if waitErr := r.waitRetry("consume", &retryAttempt, "topic consumer start consume failed: %v, reconnecting...", err); waitErr != nil {
				return waitErr
			}
			continue
		}
		...
		for !reconnect {
			...
			case err, ok := <-notifyClose:
				closeAMQPChannel(ch)
				if ok && err != nil {
					currentLogger().Infof("topic consumer channel closed: %v", err)
				}
				reconnect = true
...
func (r *MqTopic) ConsumeFailToDlx(handler MsgHandler) error {
	...
	for {
		...
		ch, err := r.openConsumerChannel()
		if err != nil {
			if waitErr := r.waitRetry("consume fail-to-dlx", &retryAttempt, "topic fail-to-dlx open channel failed: %v, reconnecting...", err); waitErr != nil {
				return waitErr
			}
			continue
		}
		...
		if err != nil {
			closeAMQPChannel(ch)
			if waitErr := r.waitRetry("consume fail-to-dlx", &retryAttempt, "topic fail-to-dlx declare topology failed: %v, reconnecting...", err); waitErr != nil {
				return waitErr
			}
			continue
		}
		...
		if err != nil {
			closeAMQPChannel(ch)
			if waitErr := r.waitRetry("consume fail-to-dlx", &retryAttempt, "topic fail-to-dlx start consume failed: %v, reconnecting...", err); waitErr != nil {
				return waitErr
			}
			continue
		}
		...
		for !reconnect {
			...
			case err, ok := <-notifyClose:
				closeAMQPChannel(ch)
				if ok && err != nil {
					currentLogger().Infof("topic fail-to-dlx channel closed: %v", err)
				}
				reconnect = true
			case msg, ok := <-deliveries:
				if !ok {
...
func (r *MqTopic) ConsumeDlx(handler MsgHandler) error {
	...
	for {
		...
		ch, err := r.openConsumerChannel()
		if err != nil {
			if waitErr := r.waitRetry("consume dlx", &retryAttempt, "topic dlx open channel failed: %v, reconnecting...", err); waitErr != nil {
				return waitErr
			}
			continue
		}
		...
		if err != nil {
			closeAMQPChannel(ch)
			if waitErr := r.waitRetry("consume dlx", &retryAttempt, "topic dlx declare topology failed: %v, reconnecting...", err); waitErr != nil {
				return waitErr
			}
			continue
		}
		...
		if err != nil {
			closeAMQPChannel(ch)
			if waitErr := r.waitRetry("consume dlx", &retryAttempt, "topic dlx start consume failed: %v, reconnecting...", err); waitErr != nil {
				return waitErr
			}
			continue
		}
		...
		for !reconnect {
			...
			case err, ok := <-notifyClose:
				closeAMQPChannel(ch)
				if ok && err != nil {
					currentLogger().Infof("topic dlx consumer channel closed: %v", err)
				}
				reconnect = true
			case msg, ok := <-deliveries:
				if !ok {
